...
async function getWeather() {
    ...
    if(!input.trim()) {
        alert("Please enter a valid state or city name");
        return;
    }
...
async function getWeather() {
 ...
 forecast
        ...
        <p id="state">Name : ${result.location.name}</p>
        <p id="state"> Region : ${result.location.region}</p>
        <h3
        >${result.current.condition.text}</h3>
        <p class="temp">Temp: ${result.current.temp_c}&degC;</p>
        <p class="temp">Feels like: ${result.current.feelslike_c}&degC;</p>
        <p class="temp">Humidity: ${result.current.humidity}&degC;</p>
    </div>`
    ...
    if (result.current.condition.text == "Sunny") {
        ...
        // Container.style.transitionDelay = "2s"
        Container.style.transitionDuration = ".01s"
    ...
    else if(result.current.condition.text == "Moderate rain") {
        Container.style.backgroundImage = "url(https://i.pinimg.com/564x/a9/87/72/a98772846d7cb84b02e01048d5003eae.jpg)"
        // Container.style.transitionDelay = "2s"
        Container.style.transitionDuration = ".01s"
        Container.style.transitionTimingFunction = "ease-in ease-out"
    }
    else if(result.current.condition.text == "Heavy rain") {
        Container.style.backgroundImage = "url(https://i.pinimg.com/564x/a9/87/72/a98772846d7cb84b02e01048d5003eae.jpg)"
        // Container.style.transitionDelay = "2s"
        Container.style.transitionDuration = ".01s"
        Container.style.transitionTimingFunction = "ease-in ease-out"
    }
    else if(result.current.condition.text == "Patchy light rain with thunder") {
        Container.style.backgroundImage = "url(https://i.pinimg.com/564x/a9/87/72/a98772846d7cb84b02e01048d5003eae.jpg)"
        // Container.style.transitionDelay = "2s"
        Container.style.transitionDuration = ".01s"
        Container.style.transitionTimingFunction = "ease-in ease-out"
    }
    else if(result.current.condition.text == "Light drizzle") {
        Container.style.backgroundImage = "url(https://i.pinimg.com/564x/a9/87/72/a98772846d7cb84b02e01048d5003eae.jpg)"
        // Container.style.transitionDelay = "2s"
        Container.style.transitionDuration = ".01s"
        Container.style.transitionTimingFunction = "ease-in ease-out"
        ...
        console.error(error);
        const data = document.getElementById("stateContainer");
        data.innerHTML = "<p>That Isn't a State Or City, Try Another Keyword or Check your internet Connection</p>"
    }
} 
// getWeather();
const retro = {name:"nosa", age:"17"}
console.log(retro.name)
=======
    } catch (error) {
        console.error(error);
    }
